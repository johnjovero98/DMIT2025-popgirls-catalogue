...
if (isset($artist_name)) {
    ...
    if ($artist_details) {
        $old_stage_name   = $artist_details['stage_name'];
        $old_description   = $artist_details['artist_description'];
        $old_record_label   = $artist_details['current_label'];
        $old_debut_year   = $artist_details['debut_year'];
        $old_album_count   = $artist_details['num_of_albums'];
        $old_latest_album   = $artist_details['latest_album_name'];
        $old_grammy_wins   = $artist_details['num_of_grammy_wins'];
        $old_billboard_no1   = $artist_details['billboard_hot_100_count'];
        $old_fandom_name = $artist_details['fandom_name'];
        $old_instagram_link   = $artist_details['instagram_link'];
        $old_instagram_followers   = $artist_details['total_ig_followers'];
    } else {
        // redirect users if there no records or if they mess up the query string
        header('Location: 404.php');
    }
}

// updated values 
$new_stage_name   =   isset($_POST['stage_name'])  ?   $_POST['stage_name']  : null;
$new_description   =   isset($_POST['description'])  ?   $_POST['description']  : null;
$new_record_label   =   isset($_POST['current_record_label'])  ?   $_POST['current_record_label']  : null;
$new_debut_year   =   isset($_POST['debut_year'])  ?   $_POST['debut_year']  : null;
$new_album_count   =   isset($_POST['albums_released'])  ?   $_POST['albums_released']  : null;
$new_latest_album   =   isset($_POST['latest_album'])  ?   $_POST['latest_album']  : null;
$new_grammy_wins   =   isset($_POST['grammy_wins'])  ?   $_POST['grammy_wins']  : null;
$new_billboard_no1   =   isset($_POST['billboard'])  ?   $_POST['billboard']  : null;
$new_fandom_name =   isset($_POST['fandom_name'])  ?   $_POST['fandom_name']  : null;
$new_instagram_link   =   isset($_POST['ig_link'])  ?   $_POST['ig_link']  : null;
$new_instagram_followers   =   isset($_POST['ig_followers'])  ?   $_POST['ig_followers']  : null;
...
if (isset($_POST['update'])) {
    ...
    // sanitize variables
    $new_stage_name   =   filter_var($new_stage_name, FILTER_SANITIZE_STRING);
    $new_description   =   filter_var($new_description, FILTER_SANITIZE_STRING);
    $new_record_label   =   filter_var($new_record_label, FILTER_SANITIZE_STRING);
    $new_debut_year   =   filter_var($new_debut_year, FILTER_SANITIZE_NUMBER_INT);
    $new_album_count   =   filter_var($new_album_count, FILTER_SANITIZE_NUMBER_INT);
    $new_latest_album   =   filter_var($new_latest_album, FILTER_SANITIZE_STRING);
    $new_grammy_wins   =   filter_var($new_grammy_wins, FILTER_SANITIZE_NUMBER_INT);
    $new_billboard_no1   =   filter_var($new_billboard_no1, FILTER_SANITIZE_NUMBER_INT);
    $new_fandom_name =   filter_var($new_fandom_name, FILTER_SANITIZE_STRING);
    $new_instagram_link   =   filter_var($new_instagram_link, FILTER_SANITIZE_URL);
    $new_instagram_followers   =   filter_var($new_instagram_followers, FILTER_SANITIZE_NUMBER_INT);

    // Validate name
    if (is_blank($new_stage_name)) {
        $message_stage_name = 'enter the new stage name';
        $form_good = false;
    } else if (!has_length_less_than($new_stage_name, 50)) {
    ...
    if (!has_length_less_than($new_description, 1000)) {
        $message_description = 'event description must have less than 500 characters';
        $form_good = false;
    }

    // validate record label
    if (is_blank($new_record_label)) {
        $message_record_label = 'enter the new record label';
        $form_good = false;
    } else if (!has_length_less_than($new_record_label, 50)) {
        $message_stage_name = 'event name must have less than 50 characters';
        $form_good = false;
    }

    // validate debut year
    if (is_blank($new_debut_year)) {
        $message_debut_year = 'enter the new debut year';
        $form_good = false;
    }
    ...
    if (is_blank($new_album_count)) {
        $message_album_count = 'enter the new album count';
        $form_good = false;
    }


    // validate latest album
    if (is_blank($new_latest_album)) {
        $message_latest_album = 'enter the new latest album';
        $form_good = false;
    }

    // validate grammy wins
    if (is_blank($new_grammy_wins)) {
        $message_grammy_wins = 'enter the new grammy win count';
        $form_good = false;
    }

    // validate billboard no.1's
    if (is_blank($new_billboard_no1)) {
        $message_billboard_no1 = 'enter the new billboard no.1 count';
        $form_good = false;
    }

    // validate fandom name
    if (is_blank($new_fandom_name)) {
        $message_fandom_name = 'enter the new fandom name';
        $form_good = false;
    }

    // validate instagram link
    if (is_blank($new_instagram_link)) {
        $message_instagram_link = 'enter the new instagram link';
        $form_good = false;
    }

    // validate instagram followers
    if (is_blank($new_instagram_followers)) {
        $message_instagram_followers = 'enter the new instagram followers count';
        $form_good = false;
    }

    // update if it passes validation
    if ($form_good === true) {
        update_arist(
            $new_stage_name,
            $new_description,
        ...
        $message_update = "succesfully updated $artist_name's info";
        $message_update .= "<p class=\"text-xl underline text-emerald-600 hover:text-pink-500\"><a href=\"view.php?artist=$new_stage_name\">see artist's details</a><p>";
    } else {
        $message_update = `there is a problem updating $artist_name's info`;
    }
        ...
        if ($file_error === 0) {
            if ($file_size < 2000000) {
                ...
                if (!file_exists($file_destination)) {
                    ...
                    $statement->bind_param("ss", $file_name_new, $artist_name);
                    $statement->execute();
